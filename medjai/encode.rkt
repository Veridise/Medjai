...
(define (decode-instruction enc #:imm imm)
  ...
  (let-values ([(flags off0-enc off1-enc off2-enc) (instruction:decode-instruction-values enc)])
    (tokamak:log "    flags: ~a, off0-enc: ~a, off1-enc: ~a, off2-enc: ~a"
                 flags
                 off0-enc
                 off1-enc
                 off2-enc)
    (define dst-register
      (if (utils:true? (bitwise-and (utils:shift-right flags DST-REG-BIT) 1)) 'fp 'ap))
    (define op0-register
      (if (utils:true? (bitwise-and (utils:shift-right flags OP0-REG-BIT) 1)) 'fp 'ap))

    ; get op1
    (define op1-addr-pat
      (list (bitwise-and (utils:shift-right flags OP1-IMM-BIT) 1)
    ...
    ; get pc_update
    (define pc-update-pat
      (list (bitwise-and (utils:shift-right flags PC-JUMP-ABS-BIT) 1)
    ...
    ; get res
    (define res-pat
      (list (bitwise-and (utils:shift-right flags RES-ADD-BIT) 1)
            (bitwise-and (utils:shift-right flags RES-MUL-BIT) 1)))
    (define res
      (cond
        [(equal? res-pat (list 1 0)) 'add]
        [(equal? res-pat (list 0 1)) 'mul]
        [(equal? res-pat (list 0 0)) (if (equal? 'jnz pc-update) 'unconstrained 'op1)]
        [(equal? res-pat (list 1 1)) 'symbolic]
        [else (tokamak:error "unrecognized res-pat")]))
    ; JNZ opcode means res must be UNCONSTRAINED
    (when (equal? 'jnz pc-update)
      (assert (equal? 'unconstrained res)))

    ; get opcode
    (define opcode-pat
      (list (bitwise-and (utils:shift-right flags OPCODE-CALL-BIT) 1)
    ...
    ; get fp_update
    (define fp-update (if (equal? 'call opcode) 'ap+2 (if (equal? 'ret opcode) 'dst 'regular)))

    (instruction:make-instruction #:off0 (- off0-enc (expt 2 (- instruction:OFFSET-BITS 1)))

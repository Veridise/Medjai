...
(provide pop-hint-from-scope
         refresh-popable-hints)
...
(define (get-caller-address-hint p #:symres? [symres? #f])
  (let* ([fp (context:context-fp (vm-cntx p))]
         ...
         ;; Use arbitrary number 10 if not symbolic
         [result (if symres? caller-addr 10)])
    (memory:memory-set! (vm-mem p) result-ptr result)))

(hash-set! hint-registry
...
(define (uint256-add-hint p)
  ...
  (define carry_low (if (>= sum_low 340282366920938463463374607431768211456) 1 0))
  (define sum_high (+ a_high b_high carry_low))
  (define carry_high (if (>= sum_high 340282366920938463463374607431768211456) 1 0))
  (memory:memory-set! (vm-mem p) (memory:rvadd fp 2) carry_low)
  (memory:memory-set! (vm-mem p) (memory:rvadd fp 3) carry_high))

(hash-set! hint-registry "starkware.cairo.common.uint256.uint256_add" (list uint256-add-hint))
...
(define (math-is-nn-hint-first p)
  ...
  (define a (memory:fromrv (memory:memory-ref (vm-mem p) (memory:rvsub fp 3))))
...
(define (math-assert-le-felt-hint-49 p)
  ...
  (assume (<= a b))
  (define res (if (and (< a range-check-builtin-bound) (< (- b a) range-check-builtin-bound)) 1 0))
  ;; small-inputs at fp
...
(hash-set! hint-registry "starkware.cairo.common.math.assert_nn" (list math-assert-nn-hint))
...
(hash-set! hint-registry "starkware.cairo.common.uint256.split_64" (list uint256-split-64))
...
(hash-set! hint-registry "starkware.cairo.common.math.split_felt" (list math-split-felt-hint-18))

(define (alloc-alloc-hint p)
  (define ap (context:context-ap (vm-cntx p)))
  (memory:memory-set! (vm-mem p) ap (memory:add-segment (vm-mem p))))

(hash-set! hint-registry "starkware.cairo.common.alloc.alloc" (list alloc-alloc-hint))

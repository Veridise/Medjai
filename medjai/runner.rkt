...
(struct runner
  ...
  #:mutable
  #:transparent
  #:reflection-name 'runner)

; raw constructor
(define (new-runner #:prog prog
                    #:layout layout
                    #:brunners brunners
                    #:osteps osteps
                    #:pfmode pfmode
  ...
  (define mem0
    (if (null? mem)
        (memory:make-memory #:prime (program:program-prime prog)
                            #:constraint-checks (list range-check)
                            #:storage-var-ids (program:program-storage-var-ids prog))
        mem))
  (define soffs0 null)
...
(define (initialize-segments p #:program-base [program-base null])
  ;; TODO should we add builtin segments here?
  (let ([mem (runner-mem p)])
    ; program segment
    (set-runner-pbase! p (if (null? program-base) (memory:add-segment mem) program-base))
    ; execution segment
    (set-runner-ebase! p (memory:add-segment mem))))

(define rngchkseg -1)
(define (set-rngcheckseg num)
  (set! rngchkseg num))

; Checks whether an rv belongs to a range_check_ptr segment
; range_check_ptr segment is from (2, >0)
; TODO we should probably do something smarter here to figure out exactly where
; in case there are multiple builtins
(define (israngechk addr)
  (tokamak:log "checking for rangechk ~a" addr)
  ;; TODO actually figure out which arg is the range_check_ptr
  (equal? (memory:rv-seg addr) rngchkseg))

(define (range-check addr value)
  (when (israngechk addr)
    (tokamak:log "adding range check constraints")
    (assume (>= (memory:fromrv value) 0))
    (assume (< (memory:fromrv value) 340282366920938463463374607431768211456))))
...
(define (setup-stack p)
  ...
  (let ([prog (runner-prog p)] [mem (runner-mem p)])
    ...
    (set-rngcheckseg (program:get-range-check-ptr-seg prog (~a (program:program-mainid prog))))
    ;; TODO set bitwise op ptr the same way
    (tokamak:log "rngcheckseg: ~a" rngchkseg)
    (for/list ([i arg-size])
...
(define (initialize-vm p
                       hint-locals
                       #:static-locals [static-locals null]
                       #:track-revert [track-revert #f])
  (define cntx
    (context:make-context #:mem (runner-mem p)
                          #:pc (runner-initpc p)
                          #:ap (runner-initap p)
                          #:fp (runner-initfp p)
  ...
  (define vm
    (vm:make-vm #:prog (runner-prog p)
                #:cntx cntx
                #:hlocals hint-locals
                #:slocals sl
                #:track-revert track-revert
                #:brunners (runner-brunners p)
                #:pbase (runner-pbase p)))
  (set-runner-vm! p vm))
...
(define (get-aps ap l)
  (match* (ap)
    [((expression (== ite) a b c))
     (begin
       (tokamak:log "ite b: ~a, c: ~a d: ~a," b c (append (get-aps b l) (get-aps c l)))
       (append (get-aps b l) (get-aps c l)))]
    [(_)
     (begin
       (tokamak:log "concrete ap: ~a" ap)
       (cons ap l))]))
...
(define (set-return-vals mem ap return-vals)
  ...
  (for ([i (length return-vals)])
    (tokamak:log "SETTING RETURN VAL ~a TO ~a "
                 (memory:rvsub ap (+ i 1))
                 (sanitize-return-val (list-ref return-vals i)))
    (memory:memory-set! mem
                        (memory:rvsub ap (+ i 1))
                        (sanitize-return-val (list-ref return-vals i)))))
  ...
  (define (do-step)
    ...
    (let ([vm0 (runner-vm p)])
      (when (! (memory:rveq (get-pc p) addr))
        (for*/all
         ([pc (memory:rv-off (context:context-pc (vm:vm-cntx (runner-vm p)))) #:exhaustive])
         (begin
           ...
           (let* ([old-ap (context:context-ap (vm:vm-cntx (runner-vm p)))])
             ...
             (when is-call
               (let* ([return-addr (memory:memory-ref (vm:vm-mem (runner-vm p))
                      ...
                      [scope (program:closest-scope (context:context-pc (vm:vm-cntx (runner-vm p)))
                                                    (runner-prog p))]
                 ...
                 (tokamak:log "ap is after call: ~a."
                              (context:context-ap (vm:vm-cntx (runner-vm p))))
                 (tokamak:log "return size: ~a" return-size)
                 (let* ([mem (vm:vm-mem (runner-vm p))]
                        [new-ap (get-new-ap mem
                                            (context:context-ap (vm:vm-cntx (runner-vm p)))
                                            return-size)])
